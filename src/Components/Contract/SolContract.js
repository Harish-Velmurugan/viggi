...
class SolContract extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      sol_name: null,
      ...
      solverView: false,
      agree: false,
      contractNumber: this.props.location.state,
  ...
  handleLoad() {
    ...
    //agree
    axios
      .get(`/contract/view-description/${this.state.contractNumber}/`)
      ...
      .then((response) => {
        ...
        if (!response.data[0].agreed && !response.data[1][0].agreed) {
